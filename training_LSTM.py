...
from utils import has_tile_to_flip
from networks_e192581 import LSTM10, LSTM11, LSTM12, LSTM13, LSTM6, LSTM7, LSTM8, LSTM9, LSTMs,LSTM2,LSTM3,LSTM4,LSTM5
...
class SampleManager():
    def __init__(self,
        ...
        from index (end_move - len_moves) to index end_move
...
class CustomDataset(Dataset):
    def __init__(self,
        ...
        if self.load_data_once4all:
            self.samples = np.zeros(
                (len(self.game_files_name)*30, self.len_samples, 8, 8), dtype=int)
            self.outputs = np.zeros(
                (len(self.game_files_name)*30, 8*8), dtype=int)
            idx = 0
            for gm_idx, gm_name in tqdm(enumerate(self.game_files_name)):
                ...
                is_black_winner = isBlackWinner(
                    game_log[1][-1], last_board_state)
                for sm_idx in range(30):
                    ...
                    self.outputs[idx] = np.array(
                        game_log[1][end_move]).flatten()
                    idx += 1
        else:
    ...
    def __getitem__(self, idx):
        ...
        if self.load_data_once4all:
            ...
            h5f = h5py.File(
                self.samples[idx].file_dir+self.samples[idx].game_name, 'r')
            game_log = np.array(
                h5f[self.samples[idx].game_name.replace(".h5", "")][:])
            h5f.close()
...
model = LSTM7(conf).to(device)
# opt = torch.optim.Adam(model.parameters(), lr=0.01)
opt = torch.optim.SGD(model.parameters(), lr=0.005, momentum=0.9)
